...
void printSyntaxTreeNode(treeNode* node, int spaceCount)
{
	...
	if (node == NULL)
	{
		//std::cout << str.ToString() << "NULL" << std::endl;
		return;
	}

	std::cout << str.ToString() << treeNodeEnum2String(node->stmt);
	if(node->content != "")
		 std::cout << "; content:"<<node->content;
	if(node->mTokenType != tokenType::NONE)
		std::cout << "; tokenType:" << tokenEnum2String(node->mTokenType);
	std::cout << std::endl;

	if(node->mLeft != NULL)
		std::cout << str.ToString() << "left:  " << std::endl; printSyntaxTreeNode(node->mLeft, spaceCount+4);
	if (node->mMiddle != NULL)
		std::cout << str.ToString() << "middle:" << std::endl; printSyntaxTreeNode(node->mMiddle,spaceCount + 4);
	if (node->mRight != NULL)
		std::cout << str.ToString() << "right: " << std::endl; printSyntaxTreeNode(node->mRight, spaceCount + 4);
	if(node->mNext != NULL)
		std::cout << str.ToString() << "next: " << std::endl; printSyntaxTreeNode(node->mNext,spaceCount + 4);
}

void printSyntaxTree(std::vector<treeNode*> outputs)
{
	for(int i = 0; i < outputs.size(); i++)
	{
		printSyntaxTreeNode(outputs.at(i), 0);
		std::cout<<"\n"<<"\n"<<"\n";
	}
}